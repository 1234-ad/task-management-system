...
import { ArrowLeftIcon, DocumentArrowUpIcon, XMarkIcon, DocumentIcon } from '@heroicons/react/24/outline';
...
const EditTask = () => {
    ...
    try {
      ...
      setTimeout(() => {
        const mockTask = {
          ...
          description: 'Implement JWT refresh tokens and improve security measures. This task involves updating the current authentication system to use refresh tokens for better security and user experience.',
          status: 'in_progress',
          priority: 'high',
          dueDate: '2025-08-15',
          assigneeId: 1,
        ...
        setFormData({
          ...
          assigneeId: mockTask.assigneeId.toString()
        });
        setExistingDocuments(mockTask.documents);
        setLoading(false);
      }, 800);
    } catch (error) {
  ...
  const handleFileUpload = (e) => {
    ...
    const totalDocuments = existingDocuments.length + newDocuments.length + validFiles.length;
    if (totalDocuments > 3) {
  ...
  const removeNewDocument = (documentId) => {
    setNewDocuments(prev => prev.filter(doc => doc.id !== documentId));
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      console.log('Updating task:', {
        ...formData,
        existingDocuments: existingDocuments.map(doc => doc.id),
        newDocuments: newDocuments.map(doc => doc.name)
      });
              ...
              <div className="mb-4">
                ...
                <div className="space-y-2">
                  {existingDocuments.map((doc) => (
                    <div key={doc.id} className="flex items-center justify-between p-3 bg-blue-50 rounded-md">
              ...
              <div className="mb-4">
                ...
                <div className="space-y-2">
                  {newDocuments.map((doc) => (
                    <div key={doc.id} className="flex items-center justify-between p-3 bg-green-50 rounded-md">
                      <div className="flex items-center space-x-3">
